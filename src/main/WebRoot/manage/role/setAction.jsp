...
<script type="text/javascript">
		...
		<c:forEach items="${moduleList}" var="module" varStatus="s">
		  ...
		  <c:forEach items="${module.actions}" var="action" varStatus="sf">
			d.add(${action.id},${module.id},"<label><input name='actions' id='${module.id}-${action.id}' type='checkbox' value='${action.id}' <c:forEach items='${role.actions}' var='actionInfo' varStatus='st'><c:if test='${action.id==actionInfo.id }'>checked</c:if></c:forEach>>${action.actionName}</label>");		
		  </c:forEach>  
		</c:forEach>
