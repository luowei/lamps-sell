...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%">
  <TBODY>
    <TR>
      ...
      <TD 
    style="FONT-SIZE: 18px; BACKGROUND-IMAGE: url(${ctx }/manage/images/bg_left_tc.gif); COLOR: white; FONT-FAMILY: system">新闻管理</TD>
    ...
    vAlign="top">
     <form action="${ctx }/manage/news/list.do" method="post">
<table>
	<tr>
		<td height="23">名称</td>
		...
		<a href="${ctx }/manage/news/toAdd.do">增加</a></td>
	</tr>
	<tr>
	<td height="22" colspan="7">
		 ...
		 <c:forEach items="${filterList}" var="condition" varStatus="s">
			...
			${condition.displayName }<a href="${ctx }/manage/news/list.do${condition.url}">删除</a>
		 </c:forEach>
	</td>
	</tr>
</table>
     ...
     <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 
      ...
      border=0>
               <TR >                
                  ...
                  <a href="${ctx }/manage/news/list.do${order }id:asc">升序</a>
                  <a href="${ctx }/manage/news/list.do${order }id:desc">降序</a></TH>
                  ...
                  <TH class=gridViewHeader scope=col>公司名</TH>
                  <TH class=gridviewHeader scope=col>
             	    排序序号
                  <a href="${ctx }/manage/news/list.do${order }createTime:asc">升序</a>
                  <a href="${ctx }/manage/news/list.do${order }createTime:desc">降序</a>				 </TH>
               ...
               <c:set var="i" value="1" />
                ...
                <TR  id="rw${i}" onmouseover="changeCss(${i})" onmouseout="recoverCss(${i})">
                  ...
                  <TD class=gridViewItem>${news.id }</TD>
                  <TD class=gridViewItem>${news.title}</TD>
                  <TD class=gridViewItem>
                  <a href="${ctx }/manage/news/detail/${news.id }.do">
                  <div style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap; margin-left: 10px; width: 100px">
                  	${news.detail}</div></a>
                  	</TD>
                  <TD class=gridViewItem>${news.type}</TD>
                  <TD class=gridViewItem>${news.status}
                  <c:if test="${news.status=='下线'}">
						<A class=cmdField
							href="${ctx }/manage/news/updateStatus.do?id=${news.id}&type=false"><img
								src="${ctx }/manage/img/icon_stop.gif"></img>
						</A>
					</c:if>
					<c:if test="${news.status=='上线'}">
						<A class=cmdField
							href="${ctx }/manage/news/updateStatus.do?id=${news.id}&type=true"><img
								src="${ctx }/manage/img/icon_run.gif"></img> </A>
					</c:if>
                  </TD>
                  <TD class=gridViewItem>${news.user.userName}</TD>
                  <TD class=gridViewItem>${news.complany.name }</TD>
                  <TD class=gridViewItem>${news.orders }
                  	<c:if test="${news.status!='下线'}">
						<a href="${ctx }/manage/news/changeOrder.do?id=${news.id }&type=up"><img
								src="${ctx }/manage/img/icon_up.gif"></img></a>
						<a href="${ctx }/manage/news/changeOrder.do?id=${news.id }&type=down"><img
								src="${ctx }/manage/img/icon_down.gif"></img> </a>
					</c:if>
                  </TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="${ctx }/manage/news/edit/${news.id}.do">编辑</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="javascript:if(confirm('确实要删除吗?'))location.href='${ctx }/manage/news/delete/${news.id}.do';">删除</A> </TD>
