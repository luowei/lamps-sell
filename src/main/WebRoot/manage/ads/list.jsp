...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%">
  <TBODY>
    <TR>
      ...
      <TD 
    style="FONT-SIZE: 18px; BACKGROUND-IMAGE: url(${ctx }/manage/images/bg_left_tc.gif); COLOR: white; FONT-FAMILY: system">广告管理</TD>
    ...
    vAlign="top">
     <form action="${ctx }/manage/ads/list.do" method="post">
<table>
	<tr>
		<td height="23">名称</td>
		...
		<a href="${ctx }/manage/ads/toAdd.do">增加</a></td>
	</tr>
	<tr>
	<td height="22" colspan="7">
		 ...
		 <c:forEach items="${filterList}" var="condition" varStatus="s">
			...
			${condition.displayName }<a href="${ctx }/manage/ads/list.do${condition.url}">删除</a>
		 </c:forEach>
	</td>
	</tr>
</table>
     ...
     <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 
      ...
      border=0>
               <TR >                
                  ...
                  <a href="${ctx }/manage/ads/list.do${order }id:asc">升序</a>
                  <a href="${ctx }/manage/ads/list.do${order }id:desc">降序</a></TH>
                  ...
                  <TH class=gridViewHeader scope=col>创建时间<br/>
                  <a href="${ctx }/manage/ads/list.do${order }createtime:asc">升序</a>
                  <a href="${ctx }/manage/ads/list.do${order }createtime:desc">降序</a>	
                  </TH>
                  <TH class=gridviewHeader scope=col>
             	    排序序号<br/>
                  <a href="${ctx }/manage/ads/list.do${order }createTime:asc">升序</a>
                  <a href="${ctx }/manage/ads/list.do${order }createTime:desc">降序</a>				 </TH>
                  <TH class=gridviewHeader scope=col>更新</TH>
                  <TH class=gridviewHeader scope=col>删除</TH>
                </TR>
               
               <c:set var="i" value="1" />
                 <c:forEach items="${adsList}" var="ads" varStatus="s">
                <TR  id="rw${i}" onmouseover="changeCss(${i})" onmouseout="recoverCss(${i})">
                  ...
                  <TD class=gridViewItem>${ads.id }</TD>
                  <TD class=gridViewItem>${ads.name}</TD>
                  <TD class=gridViewItem>
                  <c:if test="${fn:contains(ads.img ,'ads')}">
	     	 		<img id="pImg" src="${ctx }/${ads.img }"  width="100px" height="100px"/>
	     	 	</c:if>
	     	 	<c:if test="${fn:contains(ads.img ,'http')}">
	     	 		<img id="pImg" src="${ads.img}"  width="100px" height="100px"/>
	     	 	</c:if><br/>
                  </TD>
                  <TD class=gridViewItem>
                  <a href="${ctx }/manage/ads/detail/${ads.id }.do">
                  <div style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap; margin-left: 10px; width: 100px">
                  	${ads.detail}</div></a>
                  </TD>
                  <TD class=gridViewItem>${ads.status}
                  <c:if test="${ads.status=='下线'}">
						<A class=cmdField
							href="${ctx }/manage/ads/updateStatus.do?id=${ads.id}&type=false"><img
								src="${ctx }/manage/img/icon_stop.gif"></img>
						</A>
					</c:if>
					<c:if test="${ads.status=='上线'}">
						<A class=cmdField
							href="${ctx }/manage/ads/updateStatus.do?id=${ads.id}&type=true"><img
								src="${ctx }/manage/img/icon_run.gif"></img> </A>
					</c:if>
                  </TD>
                  <TD class=gridViewItem>${ads.createtime}</TD>
                  <TD class=gridViewItem>${ads.orders }
                  	<c:if test="${ads.status!='下线'}">
						<a href="${ctx }/manage/ads/changeOrder.do?id=${ads.id }&type=up"><img
								src="${ctx }/manage/img/icon_up.gif"></img></a>
						<a href="${ctx }/manage/ads/changeOrder.do?id=${ads.id }&type=down"><img
								src="${ctx }/manage/img/icon_down.gif"></img> </a>
					</c:if>
                  </TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="${ctx }/manage/ads/edit/${ads.id}.do">编辑</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="javascript:if(confirm('确实要删除吗?'))location.href='${ctx }/manage/ads/delete/${ads.id}.do';">删除</A> </TD>
