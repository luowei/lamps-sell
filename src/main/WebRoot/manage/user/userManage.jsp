	...
	<td height="22" colspan="7">
     ...
     <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 
      ...
      border=0>
               <TR >                
                  <TH>序号</TH>
                  <TH class=gridViewHeader scope=col>用户Id <br/>
                  ...
                  <TH class=gridViewHeader scope=col>角色
                  	<A class=cmdField href="${ctx }/manage/role-manage.action?id=${user.id}">所有角色</A> 
            	  </TH>
            	  <TH class=gridViewHeader scope=col>状态</TH>
            	  <TH class=gridViewHeader scope=col>在线时长</TH>
            	  <TH class=gridViewHeader scope=col>积分</TH>
                ...
                <TR id="rw${i}" onmouseover="changeCss(${i})" onmouseout="recoverCss(${i})">
            	  ...
            	  <TD class=gridViewItem>
            	  	<c:if test="${user.isEnable eq 0}">冻结用户</c:if>
            	  	<c:if test="${user.isEnable eq 1}">普通用户</c:if>
            	  	<c:if test="${user.isEnable eq 2}">管理员</c:if>
            	  	<c:if test="${user.isEnable eq 3}">申请中</c:if>
            	  </TD>
            	  <TD class=gridViewItem>${user.online }</TD>
            	  <TD class=gridViewItem>${user.score }</TD>
